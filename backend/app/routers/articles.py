...
async def get_articles(user_uuid: str, db=Depends(get_db)):
    articles : list[ArticleModel] = db.query(ArticleModel).filter(ArticleModel.user_uuid == user_uuid).all()
    return articles
...
def handle_article(db, title, user_uuid, directory, content, url=None):
    ...
    article = ArticleModel(title=title, user_uuid=user_uuid, directory=directory, summary=summary, content=content, url=url)
    db.add(article)
    db.commit()
    db.refresh(article)
...
async def create_article(article: ArticleCreateHTMLSchema = Depends(), content : str = Body(..., embed=True), db=Depends(get_db)):
    directory = db.query(DirectoryModel).filter(DirectoryModel.name == article.directory, DirectoryModel.user_uuid == article.user_uuid).first()
    if directory is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Directory not found")

    article = handle_article(db, article.title, article.user_uuid, article.directory, content, article.url)
    return article
...
async def upload_article(article: ArticleCreatePDFSchema = Depends(), db=Depends(get_db)):
    ...
    # mock summary
    article = handle_article(db, article.title, article.user_uuid, article.directory, article_content)
